...
export default function gallery(elem, onMouseDown, onMouseUp, onChange) {
  ...
  const setTransform = function(i) {
    ...
    for (let j = i - 1; j >= 0; j--) {
      l++;
      current[j].style.transform = `translate(${calculateX(newX - lastX - ratio * l, l)}px,0px) scale(${calculateScale(newX - lastX - ratio * l, l)})`;
    }
    l = 0;
    for (let k = i + 1; k < current.length; k++) {
      l--;
      current[k].style.transform = `translate(${calculateX(newX - lastX - ratio * l, l)}px,0px) scale(${calculateScale(newX - lastX - ratio * l, l)})`;
    }
  };
  ...
  const setCurrentActivePanel = function(x) {
    ...
    for (let j = i - 1; j >= 0; j--) {
      l--;
      currentArr[Number(j)].style.transform = `translate(${calculateX(ratio * l)}px,0px) scale(${calculateScale(ratio * l)})`;
    }
    l = 0;
    for (let k = i + 1; k < currentArr.length; k++) {
      l++;
      currentArr[Number(k)].style.transform = `translate(${calculateX(ratio * l)}px,0px) scale(${calculateScale(ratio * l)})`;
    }
  };
